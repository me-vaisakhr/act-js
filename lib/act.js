...
function createApp(container) {
  ...
  function createScope(name) {
    if (scopes[name]) {
      console.warn(
        `[mini-react] Scope "${name}" already exists. Use app.getScope("${name}") to access it.`
      );
      return scopes[name];
    }
  ...
  function getScope(name) {
    ...
    if (!scope) {
      console.warn(
        `[mini-react] Scope "${name}" not found. Make sure it's created first.`
      );
      return null;
    }
    return scope;
    ...
    root.querySelectorAll("[data-on]").forEach((el) => {
      ...
      for (const [event, handlerName] of Object.entries(eventMap)) {
        ...
        if (handler) {
          ...
          console.warn(
            `[mini-react] No handler registered for "${handlerName}"`
          );
        }
      }
  ...
  function destroy() {
    ...
    console.log("[mini-react] App destroyed and cleaned up.");
  }
