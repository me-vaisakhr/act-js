 ...
 * Minimal DOM patching on state change via lib/diff.js.
 *
 * MEMORY SAFETY:
 * --------------
...
function createApp(container) {
  ...
  /**
   * Re-render the app using minimal DOM patching (via lib/diff.js).
   * Only nodes that actually changed are updated in the DOM —
   * focused elements, scroll positions, and CSS transitions are preserved.
   */
  function rerender() {
    if (!renderFn) return;
    ...
    if (!container.hasChildNodes()) {
      container.appendChild(content); // initial render — nothing to diff against
    } else {
      reconcileChildren(container, content); // patch only what changed
    }
